...
def download_data(
    ...
    tiff.imwrite(os.path.join(outdir, "anchor_nuclei.tiff"), nuclei_stack)
    
    dots_stack = fov.get_image("dots", x=x_slice, y=y_slice).xarray.squeeze()
    tiff.imwrite(os.path.join(outdir, "anchor_dots.tiff"), dots_stack)
    ...
    for r in range(4):
        ...
        tiff.imwrite(os.path.join(outdir, f"r{r}_DAPI.tiff"), nuclei_stack)
        for ch in range(4):
            img = image_stack.sel({Axes.ROUND: r, Axes.CH: ch}).xarray.squeeze()
            tiff.imwrite(os.path.join(outdir, f"r{r}_ch{ch}.tiff"), img)
